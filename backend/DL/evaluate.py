    ...
    for same_list in arr:
        if version == "first":
            for word in same_list:
                ...
                res[word] = model.embedding(same_list[0], numpy=True).squeeze()
        elif version == "average":
            embed_np = np.array(
                [model.embedding(word, numpy=True).squeeze() for word in same_list])
    ...
    for same_list in arr:
        ...
        for i in range(l - 1):
            for j in range(i + 1, l):
                total_score += cos_sim(
                    model.embedding(
                        same_list[i], numpy=True).squeeze(), model.embedding(
                        same_list[j], numpy=True).squeeze())
                total_comb += 1
            ...
            for j in range(i + 1, l):
                ...
                for other_index, other_same_list in enumerate(arr):
                    ...
                    for other in other_same_list:
                        ...
                        if cos_sim(
                            model.embedding(
                                other, numpy=True).squeeze(), model.embedding(
                                same_list[i], numpy=True).squeeze()) < same_sim:
                            total_score += 1
                        if cos_sim(
                            model.embedding(
                                other, numpy=True).squeeze(), model.embedding(
                                same_list[j], numpy=True).squeeze()) < same_sim:
                            total_score += 1
    return total_score / total_comb
...
files = [
    "./model.kor2vec.ep2",
    "./model.kor2vec.ep2.with_reviews",
    "./model.kor2vec.ep0",
    "./model.kor2vec.ep0.with_reviews",
    "./model.kor2vec.ep1",
    "./model.kor2vec.ep1.with_reviews",
